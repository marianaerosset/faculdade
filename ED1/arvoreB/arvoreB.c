...
bool Balanceada (tree t){
	...
	else if (t->esq == NULL && t->dir == NULL) // se não tem filhos
		return true;
		
	else if (t->esq != NULL && t->dir != NULL) //se tem duas sub-árvores
...
bool perfeitamenteBalanceada (tree t){
	if (Vazia (t)) return true;
		...
		//para ser perfeitamente balanceada o número de nós pode diferir em, no máximo, 1
	else if (t->esq != NULL)  
...
bool buscaElemento(tree t, tipo_dado elem) {
    // Caso base: se a raiz é nula, retorna falso
    if (t == NULL)
        return false;
    
    // Se a chave está na raiz atual, retorna verdadeiro
    if (t->info.valor == elem.valor)
        return true;
        
    // Caso contrário, busca recursivamente nas subárvores esquerda e direita
